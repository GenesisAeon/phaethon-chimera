...
def run(
    ...
    destiny_predictions: bool = typer.Option(False, "--destiny-predictions", help="Print all 47 predictions"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
) -> None:
    """Run full Phaethon-Chimera simulation cycle."""
    from phaethon_chimera.system import PhaethonChimera

    console.print(f"[bold cyan]Phaethon-Chimera P35[/] — simulating {n_orbits} orbits...")
    ...
    table.add_row("Chimera R (perihelion)", f"{results['chimera']['R_perihelion']:.3f}", "0.50 ± 0.10")
    ...
    table.add_row("Emission probability", f"{results['dust']['empirical_probability']:.3f}", "0.23 ± 0.08")
    table.add_row("Geminid ZHR prediction", f"{results['geminid']['predicted_zhr']:.1f}", "120 ± 20")
...
def chimera_state(
    ...
    threshold: float = typer.Option(0.5, "--threshold", help="R threshold for chimera classification"),
) -> None:
